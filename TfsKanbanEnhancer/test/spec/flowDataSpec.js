...
describe("Snapshot", function() {
  ...
  approveIt("mixed snapshot", function(approvals) {
      var snapshot = buildSnapshot(mixedSnapshotData());
      snapshot.milliseconds = 0;
      approvals.verify(snapshot);
  });

  approveIt("Days in lane data",function(approvals){
      var dilData = buildBoardEnhancementData(mixedSnapshotData());
      approvals.verify(dilData);
  });
